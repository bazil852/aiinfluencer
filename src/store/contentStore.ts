...
interface ContentStore {
  ...
  generateVideo: (params: { influencerId: string; templateId: string; script: string; title: string }) => Promise<void>;
...
export const useContentStore = create<ContentStore>((set, get) => ({
  ...
  addContent: async (influencerId: string, title: string, script: string) => {
    try {
      const newContent = await createContent(influencerId, title, script, 'generating');
      set(state => ({
        contents: {
          ...state.contents,
  ...
  generateVideo: async ({ influencerId, templateId, script, title }) => {
    const content = await get().addContent(influencerId, title, script);
